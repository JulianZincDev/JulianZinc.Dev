...
export const CursorBlob = () => {
  ...
  const draw = () => {
    ...
    if (rect) {
      const screenOffsetY = Math.max(0, -(rect.top + rect.height / 1.55));
  ...
  useEffect(() => {
    const handlePointerMove = (e: PointerEvent) => {
      if (!scrollOffsetGroupRef.current) return;
      const rect = scrollOffsetGroupRef.current.getBoundingClientRect();
  ...
  return (
    <svg
      ...
      style={{ overflow: 'visible', maxHeight: '400px', maxWidth: '400px', zIndex: '1' }}
    >
      <g ref={scrollOffsetGroupRef} >
        ...
        <Eye eyePos={leftEye} groupRef={scrollOffsetGroupRef} />
        <Eye eyePos={rightEye} groupRef={scrollOffsetGroupRef} />
      </g>
